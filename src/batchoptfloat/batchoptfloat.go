				...
				case chunk, ok := <-work:
					...
					for offset < chunk[1]-chunk[0] {
						...
						for {
							...
							// Custom parsing of [-][d][d].d
							valRaw := line[delimIdx+1:]
							idx := 0
							sign := 1
							if valRaw[idx] == '-' {
								sign = -1
							...
							for {
								...
								val = val*10 + int(valRaw[idx]-'0')
								idx++
							}
							val *= 10
							val += int(valRaw[idx+1] - '0')
							val *= sign
							...
							if sts == nil {
								...
								stations[station] = &stats{val, val, val, 1}
							} else {
								sts[0] = min(sts[0], val)
								sts[1] = max(sts[1], val)
								sts[2] += val
